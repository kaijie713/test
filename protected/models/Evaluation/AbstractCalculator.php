    ...
    public function getElse($pdetail){
        ...
        foreach ($pdetailPost['pd_id'] as $key => $value) 
        {
            ...
            if($model->charge_type=="mkwfc"){
                ...
                $offlineRatio = $this->cal->offline_amount_sum/$this->cal->pdetail->pre_incoming;

                $amountRatio = $this->cal->offline_amount_sum/$this->cal->pdetail->pre_incoming;

                $netIncome = $this->cal->pdetail->pre_incoming/(1 - $amountRatio);
            } else if($model->charge_type=="mkyfc"){
                $offlineRatio = $this->cal->offline_amount_sum/($this->cal->pdetail->pre_incoming+$this->cal->evaformPayment->pre_ad_amount);

                $netIncome = $this->cal->pdetail->pre_incoming*(100-$this->cal->evaformPayment->ad_markting_ratio-$this->cal->evaformPayment->pre_tax_ratio)/100-$this->cal->offline_amount_sum;

                $amountRatio =1-($netIncome/($this->cal->pdetail->pre_incoming+$this->cal->evaformPayment->pre_ad_amount));

            } else if($model->charge_type=="cpswfc"){

                $offlineRatio = $this->cal->offline_amount_sum/($this->cal->pdetail->pre_incoming+$pdetail['pre_commission_amount'][$key]+$this->cal->evaformPayment->ad_amount_infact);

                $netIncome = $this->cal->pdetail->pre_incoming*(1-$offlineRatio);
                ...
                $amountRatio = 1 - ($netIncome/($this->cal->pdetail->pre_incoming+$pdetail['pre_commission_amount'][$key]+$this->cal->evaformPayment->ad_amount_infact));
            } 
